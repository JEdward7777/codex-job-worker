...
class SpeakerDiarizer:
    ...
    def __init__(
        ...
        if method == "resemblyzer":
            ...
            try:
                # Import pyannote only when needed (lazy import)
                ...
                self.pipeline = None
                self._Pipeline = Pipeline  # Store for later use
            except ImportError as e:
                raise ImportError(
    ...
    def extract_embeddings(
        ...
        logger.info("Extracting embeddings from %s audio files", len(audio_files))
        ...
        logger.info("Successfully extracted %s embeddings", len(embeddings))
    ...
    def cluster_embeddings(
        ...
        logger.info("Clustering %s embeddings using %s", len(X), self.clustering_method)
        ...
        if self.clustering_method == "hdbscan":
            try:
                ...
                logger.info("HDBSCAN found %s clusters", n_clusters)
                logger.info("Noise points: %s", n_noise)
            ...
            if self.n_clusters is None:
                ...
                logger.info("Auto-determining number of clusters (trying %s to %s)", min_k, max_k)
                ...
                logger.info("Selected %s clusters (silhouette score: %.3f)", self.n_clusters, best_score)

            # Perform K-means clustering
            kmeans = KMeans(n_clusters=self.n_clusters, random_state=42, n_init=10)
            labels = kmeans.fit_predict(X)

            logger.info("K-means clustering complete with %s clusters", self.n_clusters)
    ...
    def save_speaker_mapping(
        ...
        logger.info("Saved speaker mapping to %s", output_path)

        # Print statistics
        unique_speakers = len(set(speaker_ids))
        logger.info("Total unique speakers: %s", unique_speakers)
        ...
        for speaker_id, count in sorted(speaker_counts.items()):
            logger.info("  Speaker %s: %s samples", speaker_id, count)
    ...
    def generate_cluster_samples(
        ...
        logger.info("Generating cluster samples in %s", output_dir)
        ...
        logger.info("Generated samples for %s speakers", len(speaker_files))
...
def main():
    ...
    if metadata_path and metadata_path.exists():
        logger.info("Reading audio files from metadata: %s", metadata_path)
        with open(metadata_path, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            audio_files = [audio_dir / row['file_name'] for row in reader]
    else:
        logger.info("Scanning audio directory: %s", audio_dir)
    ...
    logger.info("Speaker IDs saved to: %s", speaker_ids_path)
    if generate_samples:
        logger.info("Cluster samples saved to: %s", samples_dir)
